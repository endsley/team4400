...
if __name__ == "__main__":
	...
	def display():
		...
		if problem_has_been_answered():
			...
			if v4 == 0: L = 'p(x=%d U y=%d)'%(v2,v3)
			else: L = 'p(x=%d U y=%d) always going to be less than'%(v2, v3)
			...
			P = v1.to_numpy()
			if v4 == 0: 
				solution = np.sum(P, axis=1)[v3] + np.sum(P, axis=0)[v2] - P[v3,v2]
			else: 
				solution = np.sum(P, axis=1)[v3] + np.sum(P, axis=0)[v2] 
