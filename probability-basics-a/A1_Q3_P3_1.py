...
if __name__ == "__main__":
    ...
    def display():
        def display_problem():
            ...
            display_new_line(r'''Given the probability table below where $X$ represents the probability of you having a good or bad date and $Y$ represents the potential topics you could talk about during a date.''')
            st.write(v1)

            display_new_line(r'''What is the probability that you have a %s date?''', (L))
        ...
        if problem_has_been_answered():
			...
			# define the solution	
            P = v1.to_numpy()
            solution = np.round(np.sum(P[:, v2]), 2)
